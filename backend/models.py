...
class AlertRule(Base):
    ...
    id          = Column(String, primary_key=True)
    name        = Column(String, nullable=False)
    metric      = Column(String, nullable=False)  # cpu/ram/disk/battery/offline
    operator    = Column(String, default="gt")    # gt/lt/eq
    threshold   = Column(Float, nullable=True)    # e.g. 90.0
    duration_minutes = Column(Integer, default=5) # must persist for X min
    target_type = Column(String, default="all")   # all/group/device
    target_id   = Column(String, nullable=True)
    action      = Column(String, default="email") # email/log
    active      = Column(Boolean, default=True)
