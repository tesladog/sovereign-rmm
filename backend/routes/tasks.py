...
async def list_tasks(db: AsyncSession = Depends(get_db)):
    ...
    for t in tasks:
        ...
        failed  = sum(1 for r in results if r.exit_code not in (None, 0))
        out.append({
...
async def create_task(data: dict, db: AsyncSession = Depends(get_db)):
    ...
    # If trigger is 'now', push to connected agents immediately via main's connections dict
    if task.trigger_type == "now":
        await _create_result_stubs(db, task)
        await _dispatch_via_main(db, task)
...
async def _dispatch_via_main(db, task: Task):
    """Push task to connected agents using main's agent_connections dict."""
    try:
        import main as _main
        connections = _main.agent_connections
        if task.target_type == "device" and task.target_id:
            targets = [task.target_id] if task.target_id in connections else []
        else:
        ...
        print(f"[Tasks] Dispatch error: {e}")
...
async def dispatch_now(task_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if task.cancelled:
        raise HTTPException(400, "Task is cancelled")
    await _create_result_stubs(db, task)
    await _dispatch_via_main(db, task)
